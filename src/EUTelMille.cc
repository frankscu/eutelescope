...
void EUTelMille::init() {
  ...
  streamlog_out ( DEBUG5 ) << "_excludePlanes_sensorIDs.size():   " << _excludePlanes_sensorIDs.size() << endl;
  streamlog_out ( DEBUG5 ) << "_excludePlanes.size():   " << _excludePlanes.size() << endl;
...
//-----------------
// By Chen
// Get _xPos, _trackResidX…… from track (EUTelDafFitter). 
...
void EUTelMille::processEvent (LCEvent * event) {
    ...
    //---------------------
    // By Chen
    // Set indexconverter. If the plane(ID) is excluded, the indexconverter[ID]=-1; 
    {
        ...
        for(size_t i = 0; i < _nPlanes; i++)
        {
            ...
            streamlog_out ( DEBUG5 ) << i << "     indexconverter:   " << indexconverter[i] << endl;
        }
    }
        ...
        for (int track = 0; track < _nTracks; track++) {
            ...
            { 
                if(_alignMode == 3)
                {
                    ...
                    for (unsigned int help = 0; help < _nPlanes; help++) 
                    {
                        ...
                        streamlog_out ( DEBUG1 ) << "excluded"<< excluded << std::endl;
                        ...
                        // ----------------
                        // By Chen
                        // It is running well in 5 planes
                    ...
                    streamlog_out( MESSAGE1 ) << " diff_mean: " << diff_mean << " _nPlanes = " << _nPlanes << " mean_n = " << mean_n << std::endl;
                    ...
                    if(ok)
                    {
                        streamlog_out( MESSAGE1 ) << "OK" << std::endl;
                        c0 = TMath::Sin(psi);
            ...
            if (_useResidualCuts == 0 || (residualsXOkay == 1 && residualsYOkay == 1)) {
                ...
                if (_alignMode == 2) {
                    ...
                    {
                        const int nLC = 4; // number of local parameters
                        const int nGL = (_nPlanes-_nExcludePlanes) * 6; // number of global parameters
                        ...
                        int nExcluded = 0;
                        streamlog_out ( MESSAGE1 ) << "nExcluded: "<<nExcluded << endl;
                        ...
                        for (unsigned int help = 0; help < _nPlanes; help++) 
                        {
                            ...
                            if (excluded == 0) {
                                ...
                                streamlog_out ( MESSAGE1 ) << "helphelp: "<<helphelp << endl;
                                for (int i = 0; i < nGL; i++ ) 
                                {
                                    derGL[i] = 0.000;
                                }
...
void EUTelMille::end() {
    ...
    if (_generatePedeSteerfile) {
        ...
        if (steerFile.is_open()) {
            ...
            streamlog_out ( DEBUG5 ) << "firstnotexcl" << firstnotexcl << endl;
            streamlog_out ( DEBUG5 ) << "lastnotexcl" << lastnotexcl << endl;
            ...
            for (unsigned int help = 0; help < _nPlanes; help++) {
                ...
                // ------------
                // By Chen
                // Set fixedPlanes parameters in "pede-steer.txt" 
                if (excluded == 0) {
    ...
    streamlog_out ( MESSAGE1 ) << "Number of data points used: " << _nMilleDataPoints << endl;
    streamlog_out ( MESSAGE1 ) << "Number of tracks used: " << _nMilleTracks << endl;
