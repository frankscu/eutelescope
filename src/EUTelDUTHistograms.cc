...
// fill tracking info:
  ...
  for(int itrack=0; itrack< _maptrackid; itrack++)
  {
    ...
    streamlog_out (MESSAGE1) << "static_cast<detMatrix>(_subMatrix[besthit]): " << static_cast<detMatrix>(_subMatrix[besthit]) << endl; 
    streamlog_out (MESSAGE1) << "_subMatrix[besthit]: " << _subMatrix[besthit] << endl; 
	...
	if (_clusterSizeX[besthit] <= HistoMaxClusterSize && _clusterSizeY[besthit] <= HistoMaxClusterSize){
	  ...
	  if (_clusterSizeX[besthit]==_clusterSizeY[besthit]){
       ...
       _ShiftXvsMX2DHisto->fill(_measuredX[bestfit], _measuredX[besthit]-_fittedX[itrack][bestfit]);
       _ShiftFXvsMX2DHisto->fill(_measuredX[besthit], _fittedX[itrack][bestfit]);
...
void EUTelDUTHistograms::bookHistos()
{
  ...
  std::string _ShiftFXvsMX2DHistoName    = "DUTshiftFXvsMX2D";
  std::string _ShiftXvsMX2DHistoName    = "DUTshiftXvsMX2D";
  ...
  shiftXTitle = "Measured - fitted X position vs X; X [mm]; #Delta X [mm]";
  std::string  shiftMXTitle = "Measured - fitted X position vs Measured X; X [mm]; #Delta X [mm]";
  std::string  shiftFMXTitle = "Measured X VS fitted X ; Measured X [mm]; Fitted X [mm]";
  ...
  _ShiftXvsMX2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(_ShiftXvsMX2DHistoName.c_str(),shiftXNBin,shiftXMin,shiftXMax,shiftVNBin,shiftVMin,shiftVMax);
  _ShiftXvsMX2DHisto->setTitle(shiftMXTitle.c_str());
  _ShiftFXvsMX2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(_ShiftFXvsMX2DHistoName.c_str(),shiftXNBin,shiftXMin,shiftXMax,shiftVNBin,shiftVMin,shiftVMax);
  _ShiftFXvsMX2DHisto->setTitle(shiftFMXTitle.c_str());
...
// using fitted position to calculate in pixel coordinates
  ...
  _bgmeasuredY.clear();
      ...
      if( meshit == 0 && meshit0 == 0 ) continue;
          ...
          streamlog_out( MESSAGE1 ) << "subMatrix: " << subMatrix <<  endl;
