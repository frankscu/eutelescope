...
EUTelDafBase::EUTelDafBase(std::string name) : marlin::Processor(name) {
  ...
  registerOptionalParameter("MinChi2OverNdof", "Minimum allowed global chi2/ndof", _minChi2, static_cast<float> ( 0.0));
...
bool EUTelDafBase::checkTrack(daffitter::TrackCandidate<float,4>& track){
  ...
  n_passedNdof++;
  if( (track.chi2 / track.ndof) > _maxChi2 || (track.chi2 / track.ndof) < _minChi2) {n_failedChi2OverNdof++; return(false); }
...
//  float logchi2max   = 5;
  ...
  _aidaHistoMap["chi2"] = AIDAProcessor::histogramFactory(this)->createHistogram1D("chi2", 300, 0, 30);
  _aidaHistoMap["logchi2"] = AIDAProcessor::histogramFactory(this)->createHistogram1D("logchi2", 140, -2, std::log10(maxNdof* _maxChi2));
  ...
  _aidaHistoMap["chi2overndof"] = AIDAProcessor::histogramFactory(this)->createHistogram1D("Chi2OverNdof", maxNdof * 50, 0, _maxChi2);
